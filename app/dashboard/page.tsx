...
import { Settings, BookOpen, PlayCircle, LogOut, Video } from "lucide-react";
...
export default function DashboardPage() {
  ...
  const [tariffLessons, setTariffLessons] = useState<Lesson[]>([]);
  ...
  useEffect(() => {
    ...
    if (user) {
      ...
      // Load tariff lessons if user has a tariff
      if (user.tariff_id) {
  ...
  const loadTariffLessons = async (tariffId: number) => {
    try {
      setLoadingLessons(true);
      const lessons = await LessonService.findAllByTariff(String(tariffId));
      setTariffLessons(lessons);
    } catch (error) {
      console.error("Tarif darslarini yuklashda xato:", error);
      setTariffLessons([]);
    } finally {
            ...
            <div className="space-y-8">
              ...
              {user.tariff_id && (
                <div>
                  ...
                  ) : tariffLessons.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                      {tariffLessons.map((lesson) => (
                        <Card key={lesson.id} className="flex flex-col">
                          ...
                          <CardFooter>
                            {lesson.video_url ? (
                              <Link href={`/watch/${encodeURIComponent(lesson.video_url)}`} className="w-full">
              ...
              {(!user.tariff_id || tariffLessons.length === 0) && (!user.courses || user.courses.length === 0) && (
